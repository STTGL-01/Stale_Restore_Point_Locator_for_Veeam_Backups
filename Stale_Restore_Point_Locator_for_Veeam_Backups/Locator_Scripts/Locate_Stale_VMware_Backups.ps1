# ============================================================
# Stale Restore-Point Report — VMware
# Detects:
#   VMware backups by Platform (EVmware / VmWare)
...
$LogPath = 'C:\Temp\VMware_Report_Log.txt'
...
Write-ReportLog "===== VMware Report Started ====="
...
function Get-VMwareRestorePoints {
    param($Backup)
...
$vmwareBackups = @(Get-VBRBackup | Where-Object {
    $platform = Get-RawPlatform -Backup $_
    $platform -match '^EVmware$|VmWare'
} | Where-Object { $_.VmCount -gt 0 })

Write-ReportLog "VMware backups detected: $($vmwareBackups.Count)"

if ($vmwareBackups.Count -gt 0) {

    foreach ($backup in $vmwareBackups) {
        ...
        $allPoints = Get-VMwareRestorePoints -Backup $backup
...
Write-Host "   Stale Restore Points - VMware Report" -ForegroundColor Cyan
Write-Host "=======================================" -ForegroundColor Cyan
Write-Host ""

if ($vmwareBackups.Count -eq 0) {

    Write-Host "No VMware backup jobs were found." -ForegroundColor Yellow
    Write-ReportLog "RESULT: no VMware backups detected"
...
if ($results.Count -eq 0 -and $ineligibleOnlyRows.Count -eq 0) {
    ...
    Write-Host "VMware jobs exist, but no eligible stale chains were found before the cutoff." -ForegroundColor Green
    Write-ReportLog "RESULT: no eligible chains and no ineligible-only rows"

    if ($encryptedBackups.Count -gt 0) {
        Write-Host ""
        Write-Host "The Following VMware Backup Jobs Could Not Be Listed As They Are Encrypted" -ForegroundColor Yellow
    ...
    if ($inaccessibleBackups.Count -gt 0) {
        Write-Host ""
        Write-Host "The Following VMware Backup Jobs Could Not Be Listed" -ForegroundColor DarkYellow
...
while ($true) {
    ...
    if ($hasEligible) {
        ...
        Write-Host "VMware exists, but no eligible chains were found before the cutoff" -ForegroundColor Green
        Write-Host ""
        Write-Host ""
        Write-Host ""
    }
