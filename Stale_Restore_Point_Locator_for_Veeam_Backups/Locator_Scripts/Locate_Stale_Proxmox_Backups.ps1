# ============================================================
# Stale Restore-Point Report — Proxmox
# Detects:
#   Proxmox backups by TypeToString
...
$LogPath = 'C:\Temp\Proxmox_Report_Log.txt'
...
Write-ReportLog "===== Proxmox Report Started ====="
...
function Get-ProxmoxRestorePoints {
    param($Backup)
...
$proxmoxBackups = @(Get-VBRBackup | Where-Object {
    $typeStr = Get-RawType -Backup $_
    $typeStr -match 'Proxmox'
})

Write-ReportLog "Proxmox backups detected: $($proxmoxBackups.Count)"

if ($proxmoxBackups.Count -gt 0) {

    foreach ($backup in $proxmoxBackups) {
        ...
        $allPoints = Get-ProxmoxRestorePoints -Backup $backup
...
Write-Host "   Stale Restore Points - Proxmox Report" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

if ($proxmoxBackups.Count -eq 0) {

    Write-Host "No Proxmox backup jobs were found." -ForegroundColor Yellow
    Write-ReportLog "RESULT: no Proxmox backups detected"
...
if ($results.Count -eq 0 -and $ineligibleOnlyRows.Count -eq 0) {
    ...
    Write-Host "Proxmox jobs exist, but no eligible stale chains were found before the cutoff." -ForegroundColor Green
    Write-ReportLog "RESULT: no eligible chains and no ineligible-only rows"

    if ($encryptedBackups.Count -gt 0) {
        Write-Host ""
        Write-Host "The Following Proxmox Backup Jobs Could Not Be Listed As They Are Encrypted" -ForegroundColor Yellow
    ...
    if ($inaccessibleBackups.Count -gt 0) {
        Write-Host ""
        Write-Host "The Following Proxmox Backup Jobs Could Not Be Listed" -ForegroundColor DarkYellow
...
while ($true) {
    ...
    if ($hasEligible) {
        ...
        Write-Host "Proxmox exists, but no eligible chains were found before the cutoff" -ForegroundColor Green
        Write-Host ""
        Write-Host ""
        Write-Host ""
    }
