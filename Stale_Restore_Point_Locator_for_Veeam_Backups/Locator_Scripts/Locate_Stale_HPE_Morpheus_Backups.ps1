...
# ✅ EARLY EXIT — no HPE Morpheus backups
if ($morpheusBackups.Count -eq 0) {

    Write-Host ""
    Write-Host "=============================================" -ForegroundColor Cyan
    Write-Host "   Stale Restore Points - HPE Morpheus Report" -ForegroundColor Cyan
    Write-Host "=============================================" -ForegroundColor Cyan
    Write-Host ""
...
if ($results.Count -eq 0 -and $ineligibleOnlyRows.Count -eq 0) {
    ...
    Write-Host "HPE Morpheus jobs exist, but no eligible stale chains were found before the cutoff." -ForegroundColor Green
...
while ($true) {
    ...
    if ($hasEligible) {
        ...
        Write-Host "HPE Morpheus exists, but no eligible chains were found before the cutoff" -ForegroundColor Green
        Write-Host ""
        Write-Host ""
        Write-Host ""
    }
